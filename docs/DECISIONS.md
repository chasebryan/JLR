...
floor applied. All media are examined and the maximum applies to every slot; an initramfs pinned to one file system
identifier never mounts another disk. Media are mounted read-only and remounted only to write. *Adopted; amends ADR-025.
Unpinned boots still depend on the real medium being attached, and say so at every boot.*

**ADR-035 - Only a proven-bad image retires a slot; only a missing file is a fresh medium.** A read error, an unreadable
state file or memory exhaustion is not evidence about a slot's content, and guessing "fresh" resets the floor to zero.
Those stop the boot or skip the slot for one boot and change nothing on disk. *Adopted.*
...
the ignored file once. *Adopted; states enrolled before this change must be re-enrolled.*
